...
taxi_dqn_config = dict(
    ...
    policy=dict(
        cuda=True,
        model=dict(obs_shape=34, action_shape=6, encoder_hidden_size_list=[128, 128]),
        ...
        other=dict(
            eps=dict(type="linear", start=1, end=0.05, decay=3000000),
...
taxi_dqn_create_config = dict(
    env=dict(type="taxi", import_names=["dizoo.taxi.envs.taxi_env"]),
