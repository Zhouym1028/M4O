...
from dizoo.m4o_hybrid.envs.m4o_hybrid_env import M4OHybridEnv

from dizoo.m4o_hybrid.config.m4o_hybrid_hppo_config import m4o_hybrid_hppo_config
...
def main(cfg, seed=0):
    ...
    collector_env = BaseEnvManager(
        env_fn=[lambda: M4OHybridEnv(cfg=cfg.env) for _ in range(collector_env_num)], cfg=cfg.env.manager
    )
    evaluator_env = BaseEnvManager(
        env_fn=[lambda: M4OHybridEnv(cfg=cfg.env) for _ in range(evaluator_env_num)], cfg=cfg.env.manager
    )
...
if __name__ == "__main__":
    main(m4o_hybrid_hppo_config, seed=0)
